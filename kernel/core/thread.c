...
void pok_thread_insert_sort_edf(uint16_t index_low, uint16_t index_high)
{
    uint32_t i=index_high,j=0;
    pok_thread_t val;
...
void pok_thread_insert_sort_wrr(uint16_t index_low, uint16_t index_high)
{   
    uint32_t i=index_high,j=0;
    pok_thread_t val;
                                       ...
                                       const uint8_t              partition_id)
{
   ...
   *thread_id = id;
   ...
   {
      pok_thread_insert_sort_edf(pok_partitions[partition_id].thread_index_low+1,id);
   }
#endif

#ifdef POK_NEEDS_SCHED_WRR
   if ((pok_partitions[partition_id].sched == POK_SCHED_WRR) && (id > pok_partitions[partition_id].thread_index_low))
   {
